...
class EdgeFXSourceImages:
    ...
    def generate_mask_definitions(self, image_stream, num_images, hold_length, transition_length, padding_frames, push, retract):
        ...
        adjusted_first_hold = hold_length - retract + push

        # Generate the first timeline with adjusted first hold value
        for i in range(num_images):
            index = i % num_images_provided
            current_hold = adjusted_first_hold if i == 0 else hold_length
            frame_distance = current_hold + transition_length
        ...
        adjusted_padding_length = padding_frames + retract - push
        ...
        if adjusted_padding_length > 0:
            ...
            padding_images_first = last_image_first_timeline.repeat(adjusted_padding_length, 1, 1, 1)
            first_timeline = torch.cat((first_timeline, padding_images_first), dim=0)
            
            padding_images_second = last_image_second_timeline.repeat(adjusted_padding_length, 1, 1, 1)
            second_timeline = torch.cat((second_timeline, padding_images_second), dim=0)

            # Append padding frames to the text outputs
            for i in range(adjusted_padding_length):
